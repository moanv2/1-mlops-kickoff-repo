...
class TestTrainModel:
    ...
    def test_returns_tuple_of_three(self, tmp_path):
        X, y = _make_data()
        result = train_model(X, y, _make_preprocessor(), "classification",
                             model_path=str(tmp_path / "model.pkl"))
        assert isinstance(result, tuple) and len(result) == 3

    def test_first_element_is_fitted_pipeline(self, tmp_path):
        X, y = _make_data()
        pipeline, _, _ = train_model(X, y, _make_preprocessor(), "classification",
                                     model_path=str(tmp_path / "model.pkl"))
        assert isinstance(pipeline, Pipeline)
        assert [s[0] for s in pipeline.steps] == ["preprocess", "model"]

    def test_test_split_shapes_are_consistent(self, tmp_path):
        X, y = _make_data(n=100)
        _, X_test, y_test = train_model(X, y, _make_preprocessor(), "classification",
                                        model_path=str(tmp_path / "model.pkl"),
                                        test_size=0.2)
    ...
    def test_artifact_is_saved_to_disk(self, tmp_path):
        X, y = _make_data()
        model_path = str(tmp_path / "model.pkl")
        train_model(X, y, _make_preprocessor(), "classification",
    ...
    def test_fitted_pipeline_can_predict(self, tmp_path):
        X, y = _make_data()
        pipeline, X_test, _ = train_model(X, y, _make_preprocessor(), "classification",
                                          model_path=str(tmp_path / "model.pkl"))
    ...
    def test_regression_uses_ridge(self, tmp_path):
        ...
        pipeline, _, _ = train_model(X, y, _make_preprocessor(), "regression",
                                     model_path=str(tmp_path / "model.pkl"))
    ...
    def test_raises_on_shape_mismatch(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Shape mismatch"):
            train_model(X, y_short, _make_preprocessor(), "classification",
                        model_path=str(tmp_path / "model.pkl"))

    def test_raises_on_unsupported_problem_type(self, tmp_path):
        X, y = _make_data()
        with pytest.raises(ValueError, match="Unsupported problem_type"):
            train_model(X, y, _make_preprocessor(), "clustering",
                        model_path=str(tmp_path / "model.pkl"))

    def test_random_state_produces_reproducible_split(self, tmp_path):
        X, y = _make_data()
        _, X_test_a, _ = train_model(X, y, _make_preprocessor(), "classification",
                                     model_path=str(tmp_path / "a.pkl"), random_state=7)
        _, X_test_b, _ = train_model(X, y, _make_preprocessor(), "classification",
                                     model_path=str(tmp_path / "b.pkl"), random_state=7)
        pd.testing.assert_frame_equal(X_test_a.reset_index(drop=True),
